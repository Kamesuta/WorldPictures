...
import com.kamesuta.mc.worldpictures.lib.Rectangle4i;
...
public abstract class GuiScrollPane extends GuiWidget {
	...
	public Rectangle windowBounds() {
		return new Rectangle(x + marginleft, y + margintop, width - marginleft - marginright,
	...
	public Rectangle scrollbarBounds() {
		...
		return new Rectangle(x + width - dim.width, y + (int) ((height - dim.height) * percentscrolled + 0.4999),
	...
	public void mouseClicked(int mx, int my, int button) {
		...
		if (button == 0 && sbar.height < height && // the scroll bar can move
													// (not full length)
				mx >= sbar.x && mx <= sbar.x + sbar.width && my >= y && my <= y + height) // in
																							// the
																							// scroll
																							// pane
		{
	...
	public void mouseMovedOrUp(int mx, int my, int button) {
		Rectangle w = windowBounds();
		if (isScrolling() && button == 0) // we were scrolling and we released
											// mouse
			scrollclicky = -1;
		else if (w.contains(mx, my))
	...
	public void drawScrollbar(float frame) {
		...
		drawRect(r.x, r.y, r.x + r.width - 1, r.y + r.height - 1, 0xFFF0F0F0);// topleft
																				// up
		drawRect(r.x + 1, r.y + 1, r.x + r.width, r.y + r.height, 0xFF555555);// bottom
																				// right
																				// down
