...
public abstract class GuiDraw {
	...
	/**
	 * Append a string in the tooltip list with TOOLTIP_LINESPACE to have a
	 * small gap between it and the next line
	 */
	public static final String TOOLTIP_LINESPACE = "\u00A7h";
	/**
	 * Have a string in the tooltip list with TOOLTIP_HANDLER +
	 * getTipLineId(handler) for a custom handler
	 */
	...
	public static void drawMultilineTip(int x, int y, List<String> list) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			Dimension d = line != null ? line.getSize()
					: new Dimension(getStringWidth(s),
